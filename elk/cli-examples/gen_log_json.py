...
FORMAT = "json" # plain, kv, json
TIMEFORMAT = "%F %T"
START_TIMESTAMP = int(time.time()) - 3 * 24 * 3600  # timestamp yesterday
FINISH_TIMESTAMP = time.time() + 3600           # timestamp now + 1 hour
...
def generate_log():
  ...
  if FORMAT == 'kv':
    ...
    res = {'timestamp': local_timestamp, 'module': module, 'submodule': submodule, 'timing': timing, 'status':status}
    if add_message:
